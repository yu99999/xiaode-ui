...
describe("测试 Select 组件", () => {
  ...
  test('测试 Select基础功能', async () => {
    const valueEl = wrapper.container.querySelector(".select-input-value-radio") as HTMLElement;
    ...
    fireEvent.click(tmp)
    expect(tmp).toHaveClass("select-option-dropdown-item-selected")
...
describe("测试 Select 多选以及hover事件", () => {
  ...
  test('测试 Select 多选功能', async () => {
    fireEvent.mouseEnter(hoverEl);
    expect(wrapper.container.querySelectorAll("tag").length).toEqual(0)
    await waitFor(() => {
    ...
    expect(wrapper.container.querySelector(".tag")).toHaveTextContent("option4")
    ...
    expect(wrapper.container.querySelectorAll(".tag").length).toEqual(2)

    fireEvent.click(wrapper.container.querySelector(".tag-close") as HTMLElement);
    expect(testProps.onChange).toHaveBeenCalledWith([{
      value: "option3",
      key: "option3",
      label: "option3"
    }])
    expect(wrapper.container.querySelector(".tag")).toHaveTextContent("option3")

    fireEvent.click(wrapper.container.querySelector(".select-option-dropdown-item.select-option-dropdown-item-selected") as HTMLElement);
    expect(testProps.onChange).toHaveBeenCalledWith([])
    expect(wrapper.container.querySelectorAll(".tag").length).toEqual(0)
